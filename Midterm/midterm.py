...
def sum_cubes(k):
    return sum(x**3 for x in range(1, k+1))
...
def sum_cubes_num_terms(n):
    return ceil((sqrt(1+8*sqrt(n))-1)/2)
...
def moving_average(measurements):
    try:
        return [(x+y+z)/3 for x, y, z in zip(measurements, measurements[1:], measurements[2:])]
    except:
        raise Exception("Moving average: {}".format(measurements))
...
def match(pattern, text):
    try:
        return pattern in text*2*max((len(pattern)//len(text))+1, 3)
    except:
        raise Exception("match:, {}, {}".format(pattern, text))
...
def share_n1(M1, M2):
    try:
        return sum(x in zip(*M2) for x in zip(*M1))>=len(M1[0])
    except:
        raise Exception("Matrixs: {}, {}".format(M1, M2))
