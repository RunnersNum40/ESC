def get_cur_hedons():
    """Returns the number of hedons that the user has accumulated so far."""
    pass

def get_cur_health():
    """Returns the number of health points that the user has accumulated so far."""
    pass

def offer_star():
    """Simulates offering the user a star for engaging in the exercise activity.
    Assume activity is a string, one of "running","textbooks", or "resting"."""
    pass

def perform_activity(activity, duration):
    """Simulates the user’s performing activity, activity for duration minutes.
    activity must be one of"running","textbooks", or"resting". duration must be a positive int"""
    pass

def star_can_be_taken(activity):
    """Returns True iff a star can be used to get more hedons if done with activity activity"""
    pass

def most_fun_activity_minute():
    """Returns the activity which would give the most hedones if preformed for one minute"""
    pass

if __name__ == '__main__':
    pass